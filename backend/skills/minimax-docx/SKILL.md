...
description: "Professional DOCX document creation, editing, and formatting using OpenXML SDK (.NET). Three pipelines: (A) create new documents from scratch, (B) fill/edit content in existing documents, (C) apply template formatting with XSD validation gate-check. Triggers: Word, docx, document, report, proposal, contract, template formatting."
triggers:
  - Word
  - docx
  - document
...
- C# source files and content.json configurations
- Intermediate DOCX outputs from validation/fix steps
- Any other scratch/working files that are not the final deliverable
NEVER write these files in the project directory or under `SKILL_DIR`. Only the final DOCX output goes to `FRONTEND_UPLOAD_DIR`. Use a unique subdirectory like `/tmp/docx_task_<timestamp>/` for each task's intermediates.

Create, edit, and format DOCX documents via CLI tools or direct C# scripts built on OpenXML SDK (.NET).

## Setup

**First time:** `bash SKILL_DIR/scripts/setup.sh` (or `powershell SKILL_DIR/scripts/setup.ps1` on Windows, `--minimal` to skip optional deps).
...
Convert `.doc` → `.docx` if needed: `bash SKILL_DIR/scripts/doc_to_docx.sh input.doc output_dir/`

Preview before editing (avoids reading raw XML): `bash SKILL_DIR/scripts/docx_preview.sh document.docx`
