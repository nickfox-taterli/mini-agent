---
name: minimax-pdf
description: "Use this skill when visual quality and design identity matter for a PDF. CREATE (generate from scratch), FILL (complete form fields), REFORMAT (apply design to an existing doc). Token-based design system with 15 document types. Triggers: make PDF, generate report, write proposal, create resume, beautiful PDF, professional document, fill form, reformat document."
license: MIT
metadata:
  version: "1.0"
...
- Python scripts and content.json files
- Cover HTML, tokens.json, and intermediate PDFs (cover.pdf, body.pdf)
- Any other scratch/working files that are not the final deliverable
NEVER write these files in the project directory or under `SKILL_DIR`. Only the final merged PDF output goes to `FRONTEND_UPLOAD_DIR`. Use a unique subdirectory like `/tmp/pdf_task_<timestamp>/` for each task's intermediates.
...
bash SKILL_DIR/scripts/make.sh run \
  ...
  --accent "#2D5F8A" \
  --content /tmp/pdf_task_<timestamp>/content.json --out FRONTEND_UPLOAD_DIR/report.pdf
```
...
# Step 2: fill
python3 SKILL_DIR/scripts/fill_write.py --input form.pdf --out FRONTEND_UPLOAD_DIR/filled.pdf \
...
bash SKILL_DIR/scripts/make.sh reformat \
  --input source.md --title "My Report" --type report --out FRONTEND_UPLOAD_DIR/output.pdf
```
...
bash SKILL_DIR/scripts/make.sh demo    # build a sample PDF (output in /tmp/)
```
